...
public class MatrixSpigotBridge extends JavaPlugin implements Listener {
	...
	private SchedulerAdapter.TaskWrapper establishConnection;
	private SchedulerAdapter.TaskWrapper matrixPollerTask;
	private SchedulerAdapter.TaskWrapper topicUpdaterTask;
	...
	public void startBridgeAsync(CommandSender sender, Consumer<Boolean> callback) {
		...
		if (establishConnection != null) {
			...
			if (sender instanceof Player) {
				SchedulerAdapter.runGlobal(this, () -> sender.sendMessage(
						"§e[MatrixSpigotBridge] §cThere is already a connection attempt in progress, forcefully reconnecting..."));
			}
			try {
			...
			try {
				if (!messages.isEmpty()) {
					messages.forEach(o -> {
						...
						String sender_address = matrix.getDisplayName(obj.getString("sender"),
								!config.cacheMatrixDisplaynames);
		...
		if (!tokenFile.exists()) {
			...
			try {
				...
				logger.log(Level.SEVERE,
						"This will not prevent the bridge from working, but new access tokens will be generated every time.");
			}
		} else {
		...
		establishConnection = SchedulerAdapter.runAsync(this, () -> {
			...
			if (pingMs < 0) {
				logger.warning("Matrix homeserver (" + config.matrixServer
						+ ") reachability check failed; proceeding with connection attempt...");
			}
			...
			if (authenticated) {
				...
				if (validRoomCount > 0 && matrix.isConnected()) {
					SchedulerAdapter.runGlobal(this, () -> {
						...
						try {
							...
							if (config.getMatrixPassword() != null && !config.getMatrixPassword().isEmpty())
								logger.info(
										"Logged in with token and saved it to access.yml. You can now remove the password from config.yml if you wish to.");
						} catch (IOException e) {
					...
					logger.severe(
							"Could not connect to any configured Matrix rooms! Please check your room IDs in config.yml and run /msb restart!");
				}
			}
			...
			if (connected) {
				...
				if (config.matrixRooms != null && !config.matrixRooms.isEmpty()) {
					logger.info(
							"Connected to Matrix server as " + config.matrixUserId + " in room(s): " + purposeRooms);
				} else {
					logger.info(
							"Connected to Matrix server as " + config.matrixUserId + " in room " + config.matrixRoomId);
				}
				...
				SchedulerAdapter.runGlobal(this, () -> {
					...
					matrixPollerTask = SchedulerAdapter.runAsyncTimer(this, poller, 0, config.matrixPollDelay * 20);
				});
			} else {
				logger.log(Level.SEVERE,
						"Could not connect to server! Please check your configuration and run /msb restart!");
				if (sender != null) {
					SchedulerAdapter.runGlobal(this, () -> sender.sendMessage(
							"§e[MatrixSpigotBridge] §cCould not connect to server! Please check your configuration and run /msb restart!"));
				}
			}
			...
			if (callback != null) {
				...
				SchedulerAdapter.runGlobal(this, () -> callback.accept(finalConnected));
			}
	...
	public void checkAndWarnRoomPowerLevels(CommandSender sender) {
		...
		boolean topicUpdateActive = config.matrixTopicUpdateInterval > -1 && config.hasAnyRoomTopicPool();
		boolean topicShutdownActive = config.hasAnyRoomTopicShutdown();

		if (topicUpdateActive || topicShutdownActive) {
			java.util.Set<String> targetRooms = new java.util.LinkedHashSet<>();
			targetRooms.addAll(config.getRoomTopicsByRoomId().keySet());
			targetRooms.addAll(config.getShutdownTopicsByRoomId().keySet());
			String chatRoomId = config.getRoomIdForPurpose(MessagePurpose.CHAT);
			...
			for (String rId : targetRooms) {
				...
				if (botLevel >= 0 && botLevel < requiredLevel) {
					...
					String warnMsg = "Room topic updating is enabled, but bot user power level (" + botLevel +
							...
							"). Topic updates will fail unless the bot is granted Moderator (level " + requiredLevel
							+ "+) in Matrix room settings.";
	...
	private boolean tryPasswordLogin(Matrix matrix, CommandSender sender) {
		...
		if (matrixPassword != null && !matrixPassword.isEmpty()) {
			...
			logger.severe(
					"No valid access token or password found! Please set a password in config.yml or run /msb restart to generate a new access token.");
			if (sender != null) {
				SchedulerAdapter.runGlobal(this, () -> sender.sendMessage(
						"§e[MatrixSpigotBridge] §cNo valid access token or password found! Please set a password in config.yml or run /msb restart to generate a new access token."));
			}
			return false;
		}
	}
	...
	public void updateRoomTopicAsync(Consumer<Boolean> callback) {
		...
		Runnable roomTopicUpdater = new Runnable() {
			private final java.util.Map<String, Integer> nextTopicIndices = new java.util.HashMap<>();

			@Override
			public void run() {
				boolean randomize = config.getFormatSettingBool("randomize_topic");
				Map<String, List<String>> roomTopics = config.getRoomTopicsByRoomId();
				boolean overallSuccess = true;

				for (Map.Entry<String, List<String>> entry : roomTopics.entrySet()) {
					String rId = entry.getKey();
					List<String> pool = entry.getValue();
					if (rId == null || rId.isEmpty() || pool == null || pool.isEmpty())
						continue;

					String room_topic;
					if (randomize) {
						int idx = (int) (Math.random() * pool.size());
						room_topic = pool.get(idx);
					} else {
						int idx = nextTopicIndices.getOrDefault(rId, 0);
						room_topic = pool.get(idx % pool.size());
						nextTopicIndices.put(rId, (idx + 1) % pool.size());
					}
					...
					if (room_topic != null && !room_topic.isEmpty()) {
						...
						boolean res = matrix.setRoomTopic(rId, room_topic);
						if (!res)
				...
				// Notify callback on main thread
				if (callback != null) {
					SchedulerAdapter.runGlobal(MatrixSpigotBridge.this, () -> callback.accept(success));
				}
			}
		};
		if (config.matrixTopicUpdateInterval > 0 && config.hasAnyRoomTopicPool()) {
			SchedulerAdapter.runGlobal(this, () -> {
				topicUpdaterTask = SchedulerAdapter.runAsyncTimer(this, roomTopicUpdater, 0,
						config.matrixTopicUpdateInterval * 60 * 20);
			});
		} else if (config.matrixTopicUpdateInterval == 0) {
			// If no topic update interval is set, run once immediately
			topicUpdaterTask = SchedulerAdapter.runAsync(this, roomTopicUpdater);
		} else if (config.matrixTopicUpdateInterval < 0) {
			// If negative, do not run the task, just callback true
			if (callback != null) {
				SchedulerAdapter.runGlobal(this, () -> callback.accept(true));
			}
		}
	}
	...
	public void sendMessageToMinecraft(String format, String event_id, String message, String formattedMessage,
			Player player) {
		sendMessageToMinecraft(format, event_id, message, formattedMessage, player, "???");
	}

	public void sendMessageToMinecraft(String format, String event_id, String message, String formattedMessage,
			Player player, String defaultPlayername) {
		sendMessageToMinecraft(format, event_id, message, formattedMessage, player, defaultPlayername,
				config.getRoomIdForPurpose(MessagePurpose.CHAT));
	}

	public void sendMessageToMinecraft(String format, String event_id, String message, String formattedMessage,
			Player player, String defaultPlayername, String eventRoomId) {
		SchedulerAdapter.runGlobal(this, () -> {
			String fmt = format;
			if (config.canUsePapi)
				fmt = formatter.replacePlaceholderAPI(player, fmt);
			...
			if (config.getFormatSettingBool("reserialize_matrix") && !formattedMessage.isEmpty())
				msg = formatter.matrixHTMLToMinecraft(formattedMessage);

			Bukkit.broadcastMessage(formatter.replaceTimePlaceholders(fmt)
	...
	public void onEnable() {
		...
		if (config.bstatsConsent && !config.isFirstRun) {
			...
			logger.info(
					"bstats for MatrixSpigotBridge has been enabled. You can opt-out by disabling bstats in the plugin config.");
		}
	...
	public void onDisable() {
		...
		Map<String, String> shutdownTopics = config.getShutdownTopicsByRoomId();
		if ((!stop_message.isEmpty() || !shutdownTopics.isEmpty()) && matrix != null) {
			final String msg = !stop_message.isEmpty() ? formatter.replaceTimePlaceholders(stop_message) : null;
			final Map<String, String> formattedShutdownTopics = new java.util.HashMap<>();
			for (Map.Entry<String, String> entry : shutdownTopics.entrySet()) {
				String rId = entry.getKey();
				...
				if (topic != null && !topic.isEmpty()) {
					...
					topic = formatter.replaceTimePlaceholders(topic);
					formattedShutdownTopics.put(rId, topic);
				}
			}
			...
			Thread shutdownThread = new Thread(() -> {
				try {
					...
					for (Map.Entry<String, String> entry : formattedShutdownTopics.entrySet()) {
						matrix.setRoomTopic(entry.getKey(), entry.getValue());
					}
				} catch (Exception ignored) {
				}
			});
